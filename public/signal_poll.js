...
(function($)
{
  ...
  var onPollTimeout = function()
  {
    ...
    if ((outstandingPalRequests < maxPalRequests) && (readyQueue.length <= 1 || fetchCount > 0))
    {
      if (fetchCount > 0)
      {
        ...
        var params = {facet:"network,0,1,2", facets:"industry,company", count: fetchCount};
      ...
      {
        var params = {facet:"network,0,1,2", facets:"industry,company", after: after, poll:"true"};
  ...
  var onBacklogTimeout = function()
  {
    ...
    while ((outstandingPalRequests < maxPalRequests) && (backlogQueue.length > 0 && (nodeCount < maxNodes)))
    {
      ...
      var params = {facet:"network,0,1,2", facets:"industry", count:10, keywords: query};
  ...
  var processPollCountFetchResults = function(result)
  {
    ...
    for (var i = 0; i < fetchCount && visibleQueue.length > 0; i++)
    {
      update = visibleQueue.shift();
      deleteNode(sys.getNode(update.id));
    }
